...
CREATE TRIGGER IF NOT EXISTS refund_claim_guard BEFORE INSERT ON refund_requests WHEN NEW.Status<>'failed' AND NOT EXISTS(SELECT 1 FROM refund_requests WHERE RequestKey=NEW.RequestKey) BEGIN  SELECT CASE WHEN NOT EXISTS(SELECT 1 FROM sales WHERE Id=NEW.BusinessId AND Status IN ('completed','partially_refunded')  AND RefundedCents=NEW.SnapshotRefunded AND NEW.AmountCents<=TotalCents-RefundedCents AND upper(Currency)=NEW.Currency)  THEN RAISE(ROLLBACK,'Sale balance changed. Reload the sale before refunding.') END;  SELECT CASE WHEN NEW.AmountCents>(SELECT COALESCE(SUM(AmountCents),0) FROM sale_payments WHERE SaleId=NEW.BusinessId AND Method=NEW.Method AND Status='succeeded')  -(SELECT COALESCE(SUM(AmountCents),0) FROM refunds WHERE SaleId=NEW.BusinessId AND Method=NEW.Method AND Status='completed')  THEN RAISE(ROLLBACK,'Refund exceeds the amount paid by this method.') END;  SELECT CASE WHEN NEW.Method='card' AND NOT EXISTS(SELECT 1 FROM sale_payments WHERE Id=NEW.PaymentRowId AND SaleId=NEW.BusinessId AND ConnectPaymentId=NEW.PaymentId AND Method='card' AND Status='succeeded' AND AmountCents>=NEW.AmountCents)  THEN RAISE(ROLLBACK,'Original card payment is not refundable.') END;  SELECT CASE WHEN json_array_length(NEW.LinesJson)=0 OR EXISTS(SELECT 1 FROM json_each(NEW.LinesJson) l LEFT JOIN sale_items i ON i.Id=json_extract(l.value,'$.saleItemId')  WHERE i.Id IS NULL OR i.SaleId<>NEW.BusinessId OR abs(i.RefundedQty-json_extract(l.value,'$.expectedRefundedQty'))>0.000000001  OR json_extract(l.value,'$.qty')<=0 OR json_extract(l.value,'$.qty')>i.Qty-i.RefundedQty+0.000000001)  THEN RAISE(ROLLBACK,'Returned quantities changed. Reload the sale.') END;  SELECT CASE WHEN NEW.ShiftId IS NOT NULL AND NOT EXISTS(SELECT 1 FROM shifts WHERE Id=NEW.ShiftId AND Status='open')  THEN RAISE(ROLLBACK,'The cash refund shift has closed.') END; END;
CREATE TRIGGER IF NOT EXISTS refund_apply AFTER UPDATE OF Status ON refund_requests WHEN NEW.Status='succeeded' AND OLD.Applied=0 AND OLD.Status<>'succeeded' BEGIN  SELECT CASE WHEN NOT EXISTS(SELECT 1 FROM sales WHERE Id=NEW.BusinessId AND RefundedCents=NEW.SnapshotRefunded)  THEN RAISE(ROLLBACK,'Refund needs reconciliation: local sale balance changed.') END;  INSERT INTO refunds(SaleId,Reference,Method,AmountCents,Reason,ItemsJson,ConnectRefundId,ShiftId,CreatedBy)  VALUES(NEW.BusinessId,(SELECT Reference FROM sales WHERE Id=NEW.BusinessId)||'-RF-'||NEW.Id,NEW.Method,NEW.AmountCents,json_extract(NEW.IntentJson,'$.reason'),NEW.LinesJson,NEW.ProviderRefundId,NEW.ShiftId,NEW.CreatedBy);  UPDATE sale_items SET RefundedQty=RefundedQty+(SELECT json_extract(value,'$.qty') FROM json_each(NEW.LinesJson) WHERE json_extract(value,'$.saleItemId')=sale_items.Id)  WHERE Id IN (SELECT json_extract(value,'$.saleItemId') FROM json_each(NEW.LinesJson));  UPDATE products SET StockQty=StockQty+(SELECT SUM(json_extract(value,'$.qty')) FROM json_each(NEW.LinesJson) WHERE json_extract(value,'$.productId')=products.Id),UpdatedAt=CURRENT_TIMESTAMP  WHERE Id IN (SELECT json_extract(value,'$.productId') FROM json_each(NEW.LinesJson)) AND TrackInventory=1;  INSERT INTO inventory_movements(ProductId,Delta,Reason,Reference,Note,CreatedBy)  SELECT p.Id,json_extract(l.value,'$.qty'),'refund',(SELECT Reference FROM sales WHERE Id=NEW.BusinessId),'Confirmed refund '||NEW.RequestKey,NEW.CreatedBy  FROM json_each(NEW.LinesJson) l JOIN products p ON p.Id=json_extract(l.value,'$.productId') WHERE p.TrackInventory=1;  UPDATE sales SET RefundedCents=RefundedCents+NEW.AmountCents,  Status=CASE WHEN RefundedCents+NEW.AmountCents>=TotalCents THEN 'refunded' ELSE 'partially_refunded' END,UpdatedAt=CURRENT_TIMESTAMP WHERE Id=NEW.BusinessId;  INSERT INTO sale_events(SaleId,Type,Message,DataJson,CreatedBy)  VALUES(NEW.BusinessId,'refund',NEW.Method||' refund confirmed',json_object('requestKey',NEW.RequestKey,'refundId',NEW.ProviderRefundId,'reason',json_extract(NEW.IntentJson,'$.reason')),NEW.CreatedBy);  UPDATE refund_requests SET Applied=1,UpdatedAt=CURRENT_TIMESTAMP WHERE Id=NEW.Id; END;
