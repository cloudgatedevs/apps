...
const NavLinks = ({ onNavigate }) => {
  const groups = NAV.reduce((acc, item) => { (acc[item.group] ||= []).push(item); return acc; }, {});
  return (
    <nav className="flex flex-col gap-1">
      {Object.entries(groups).map(([group, items], gi) => (
        <div key={group} className={gi ? 'mt-3' : ''}>
          <p className="px-2.5 pb-1.5 text-[10px] font-semibold uppercase tracking-[0.16em] text-mist-dim">{group}</p>
