...
/**
 * @param {File} file
 * @param {{ path?: string }} [opts]  folder shown in the hub Files grid, e.g. "pos/products"
 * @returns {Promise<{ id: string, fileId: string, url: string, thumbUrl: string, name: string, size: number }>}
 */
export async function uploadImage(file, { path = 'pos/products' } = {}) {
  const token = auth.getAccessToken();
  if (!idpApiUrl || !auth.tenancyName) throw new Error('IdP is not configured.');
 ...
 */
export async function listImages({ path = 'pos/products', skip = 0, take = 100 } = {}) {
  const token = auth.getAccessToken();
