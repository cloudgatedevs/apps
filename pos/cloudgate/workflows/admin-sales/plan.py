...
return ("SELECT (SELECT json_group_object(Key, Value) FROM settings) AS SettingsJson, "
        ...
        "  'ItemsJson', (SELECT json_group_array(json_object('Id', i.Id, 'ProductId', i.ProductId, 'Name', i.Name, 'Qty', i.Qty, 'LineTotalCents', i.LineTotalCents, 'RefundedQty', i.RefundedQty)) FROM sale_items i WHERE i.SaleId = s.Id), "
        "  'Succeeded', (SELECT json_object('Id', p.Id, 'ConnectPaymentId', p.ConnectPaymentId, 'AmountCents', p.AmountCents) FROM sale_payments p WHERE p.SaleId = s.Id AND p.Method = 'card' AND p.Status = 'succeeded' ORDER BY p.Id DESC LIMIT 1)) "
