const SETTINGS = { logo_url: 'Logo', icon_url: 'App icon', favicon_url: 'Favicon', hero_image_url: 'Homepage hero', about_image_url: 'Editorial image' };
export const MEDIA_FOLDERS = { 'events/events': 'Event artwork', 'events/staff': 'Event staff', 'events/branding': 'Branding', 'events/library': 'General library' };
export const folderLabel = path => MEDIA_FOLDERS[path] || path.slice('events/'.length);
export function isEventsFolder(path) {
  return typeof path === 'string' && path.startsWith('events/') && path.split('/').every(part => part && part !== '.' && part !== '..' && !part.includes('\\'));
}

function imageKeys(value) {
  if (!value) return [];
  try {
    const url = new URL(String(value).trim(), 'https://events.invalid');
...
export function mediaRows(files, data) {
  if (!Array.isArray(data?.services) || !data?.settings) throw new Error('Reload events data before managing media.');
  const references = (data.mediaReferences || []).map(r => ({keys:imageKeys(r.url),label:r.label,page:'events'}));
  for (const person of data.staff || []) references.push({ keys: imageKeys(person.image_url), label: person.name + ' (team)', page: 'staff' });
  for (const service of data.services) {
    // Retired services no longer display their photo. Hidden services still need theirs.
    if (Number(service.deleted)) continue;
    references.push({ keys: imageKeys(service.image_url), label: service.name + (Number(service.active) ? '' : ' (hidden)'), page: 'events' });
  }
  for (const [key, label] of Object.entries(SETTINGS)) references.push({ keys: imageKeys(data.settings[key]), label, page: 'settings' });
  return files.filter(file => isEventsFolder(file.path)).map(file => {
...
export function deletionCandidates(ids, files, data) {
  ...
  return [...new Set(ids.map(String))].map(id => {
    ...
    if (!row) throw new Error('An image is no longer in the Events library. Refresh and select your images again.');
    if (row.inUse) throw new Error(`${row.name} is now in use. Remove it from its event, staff or branding setting before deleting it.`);
    return row;
  });
}

// The host caps each page. Read every page before filtering by app folder so
// events images remain visible even when other apps have a large library.
