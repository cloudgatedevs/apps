...
import { isEventsFolder, loadMediaPages } from './media-model';

export async function saveBrandImage(blob, name, path = 'events/branding') {
...
export async function listEventsImages() {
  const files = await loadMediaPages(options => preview ? call('list', options, true, 'branding-media') : listImages(options));
  return files.filter(file => isEventsFolder(file.path));
}

export function removeEventsImage(id) {
