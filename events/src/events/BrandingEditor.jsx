...
import { BRAND_DEFAULTS, THEME_PRESETS, THEME_KEYS, matchingPreset, brandIdentity, imageUrl, themeTokens } from './branding-model';
...
export default function BrandingEditor({ value, change }) {
  const brand = brandIdentity(value), selectedPreset = matchingPreset(value);
  return <>
<section className="panel settings-group"><div className="theme-section-heading"><div><span className="eyebrow">MAKE IT YOURS</span><h3>Choose your atmosphere.</h3></div><span className="theme-current">{selectedPreset?.name || 'Custom palette'}</span></div><p className="muted">Start with a curated palette, then make it your own. Your colours flow through the website, ticket portal and back office.</p><div className="theme-presets" role="group" aria-label="Theme presets">{THEME_PRESETS.map(p=><button key={p.name} type="button" className="theme-preset" aria-pressed={selectedPreset?.name===p.name} onClick={()=>THEME_KEYS.forEach(k=>change(k,p[k]))}><span className="theme-swatch" style={{background:p.theme_background}}><i style={{background:p.theme_primary}}/><i style={{background:p.theme_accent}}/><i style={{background:p.theme_primary}}/></span><strong>{p.name}</strong><small>{p.description}</small>{selectedPreset?.name===p.name&&<span className="theme-selected">Selected</span>}</button>)}</div><h4>Fine-tune your colours</h4><p className="muted">Every colour stays editable. Text and surface shades adapt automatically for contrast.</p><div className="brand-colours"><Colour label="Primary colour" value={value.theme_primary} change={v => change('theme_primary', v)}/><Colour label="Accent colour" value={value.theme_accent} change={v => change('theme_accent', v)}/><Colour label="Page background" value={value.theme_background} change={v => change('theme_background', v)}/></div><button type="button" className="text-link" onClick={() => ['theme_primary', 'theme_accent', 'theme_background'].forEach(key => change(key, BRAND_DEFAULTS[key]))}>Restore default colours</button>
    <div className="branding-preview" style={themeTokens(value)} aria-label="Branding preview"><div className="brand-browser-tab"><img src={brand.favicon} alt=""/><span>{brand.name}</span><small>Preview</small></div><div className="brand-preview-body"><Brand settings={value} small/><span className="eyebrow">YOUR NEXT GOOD THING</span><h2>{value.tagline || 'Good things happen together.'}</h2><p>{value.description || 'Practical events for curious people.'}</p><div className="actions"><span className="button">Explore events <ArrowUpRight size={15}/></span><span className="brand-preview-tag"><Leaf size={15}/> Be part of the moment</span></div></div></div><p className="muted brand-preview-note">Preview only. Save settings to apply this palette across your events.</p></section>
    <section className="panel settings-group"><h3>Branding</h3><p className="muted">Your identity across the events site, workspace and browser tab.</p><div className="form-grid"><Field label="App name" hint="Leave blank to use your business name."><input value={value.app_name} maxLength={120} placeholder={value.name || 'Events'} onChange={e => change('app_name', e.target.value)}/></Field><Field label="Short name" hint="Used when attendees add the site to their home screen."><input value={value.app_short_name} maxLength={30} placeholder={brand.name} onChange={e => change('app_short_name', e.target.value)}/></Field></div>
    ...
    <section className="panel settings-group"><h3>Website imagery</h3><p className="muted">Upload your images to Files &amp; media, then choose them from Library below. Save settings to apply them to the public website.</p><div className="brand-assets"><Asset label="Homepage hero" hint="The full-width image behind the homepage headline. A wide landscape image works best." value={value.hero_image_url} change={v => change('hero_image_url', v)} path="events/branding" aspect={2}/><Asset label="Editorial image" hint="The image beside “A place in the moment” on the homepage." value={value.about_image_url} change={v => change('about_image_url', v)} path="events/branding" aspect={1.5}/></div><p className="muted">For event cards, event pages and the next-event panel, choose Event artwork under Events → Edit event.</p></section>
  </>;
}
