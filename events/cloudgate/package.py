...
def snapshot(name='Snapshot'):
    ...
    if not rows:raise Exception('Events database could not be loaded.')
    raw=rows[0]['snapshot']
...
def payment_function(code,engine=False):return fn(PAYMENT_RETURN+'\n'+code,engine)

graphs=[]
def graph(route,steps,anonymous=False,schedule=False):
    eid=str(uuid.uuid5(uuid.NAMESPACE_URL,'cloudgate-events/v1/'+route));nodes=[];names={}
...
groups={'catalog':['catalog'],'workspace':['workspace'],'events':['event-save','event-publish','event-archive','event-cancel','tier-save'],'orders':['reserve','order-cancel'],'waitlist':['waitlist-join','waitlist-notify'],'checkin':['check-in','check-in-undo'],'staff':['staff-save'],'settings':['settings']}
...
graph('checkout',[('IdpAuth','idp',None),('Snapshot','database',snap),('Prepare','function',payment_function("o=Engine(snapshot(),obj('IdpAuth',{})).dispatch(dict(request(),op='checkout-prepare'))\no['url']=payment_return_base(o['url'],value('Header_Origin'))\nreturn encoded(o)",True)),('Amount','function',extract('amount')),('Currency','function',extract('currency')),('Reference','function',extract('ref')),('Email','function',extract('email')),('ReturnUrl','function',fn("return obj('Prepare')['url'].rstrip('/')+'/account?payment='+obj('Prepare')['ref']")),('WalletCreate','wallet',None,{'params':dict(Param1='create',Param2='${Amount}',Param3='${Currency}',Param4='Events ${Reference}',Param5='${Reference}',Param6='${Email}',Param7='${ReturnUrl}',Param8='${ReturnUrl}',Param9='events-${Reference}')}),('AfterWallet','database',snap),('Plan','function',fn("p=obj('WalletCreate',{});o=obj('Prepare')\nsql,result=plan(snapshot('AfterWallet'),dict(op='_checkout-save',ref=o['ref'],payment_id=str(p.get('Id','')),payment_url=p.get('PaymentUrl','')),internal=True)\nreturn sql",True)),('Run','database','${Plan}'),('Shape','function',shape)])
...
databases=[dict(Id=R['databaseId'],Name='events_sandbox',Type=0,FileId=R['fileId'],FileProdIdGuid=None,ProjectId=R['projectId'],SQLScript=schema,SQLScriptProd=schema),dict(Id=R['databaseProdId'],Name='events_production',Type=0,FileId=R['fileProdId'],FileProdIdGuid=R['fileProdId'],ProjectId=R['projectId'],SQLScript=schema,SQLScriptProd=schema)]
bundle=dict(Projects=[dict(Name='Cloudgate Events',Path='events',IsPrivate=False,Template=dict(Endpoints=[g['Endpoint'] for g in graphs],Nodes=[n for g in graphs for n in g['Nodes']],Keys=[],Databases=databases,WebSockets=[],TestRequests=[],Snippets=[]))])
