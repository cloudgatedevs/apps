...
def upload(directory, content, name, path='events/branding'):
    if not re.fullmatch(r'events/(?:[A-Za-z0-9_-]+/)*[A-Za-z0-9_-]+', str(path)):
...
def listing(directory, skip=0, take=36, path='*'):
    ...
    for file in files:
        ...
        item.setdefault('path', 'events/branding')
...
def delete(directory, key, references=()):
    ...
    if not str(item.get('path', 'events/branding')).startswith('events/'):
