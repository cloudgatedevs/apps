...
import local_media
ROOT=Path(__file__).parent;DB=ROOT.parent/'.local/events.sqlite';LOCK=threading.RLock()
IDENTITIES={'admin':dict(Id='1',Role='Admin',IsActive=True,Name='Alex Morgan',Email='alex@example.invalid'),'attendee':dict(Id='2',Role='User',IsActive=True,Name='Jordan Williams',Email='jordan@example.invalid'),'staff':dict(Id='3',Role='User',IsActive=True,Name='Alex Morgan',Email='alex@example.invalid'),'other':dict(Id='4',Role='User',IsActive=True,Name='Other Attendee',Email='other@example.invalid')}
def connect():
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            if host not in ('localhost','127.0.0.1') or (origin.hostname and origin.hostname not in ('localhost','127.0.0.1')) or self.headers.get('X-Events-Preview')!='1':return self.send(403,dict(error='Local preview only.'))
            ...
            if route=='/api/checkout':
                ...
                v=Engine(state(),identity).dispatch(dict(d,op='refund-prepare'));run(dict(op='_refund-claim',prepared=v),internal=True);result=run(dict(op='_refund-apply',request_key=v['request_key'],provider=dict(PaymentId=v['payment_id'],Amount=v['amount'],Currency=v['currency'],IdempotencyKey='events-refund-'+v['request_key'],RefundId='sim-'+v['request_key'],Status='succeeded')),internal=True)
                ...
                elif op=='upload':result=local_media.upload(DB.parent/'media',d['content'],d['name'],d.get('path','events/branding'))
...
if __name__=='__main__':
    ...
    print('Events preview API on port',args.port,flush=True);ThreadingHTTPServer(('127.0.0.1',args.port),Handler).serve_forever()
