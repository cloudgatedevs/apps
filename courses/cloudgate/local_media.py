...
def upload(directory, content, name, path='courses/branding'):
    if not re.fullmatch(r'courses/(?:[A-Za-z0-9_-]+/)*[A-Za-z0-9_-]+', str(path)):
...
def listing(directory, skip=0, take=36, path='*'):
    ...
    for file in files:
        ...
        item.setdefault('path', 'courses/branding')
...
def delete(directory, key, references=()):
    ...
    if not str(item.get('path', 'courses/branding')).startswith('courses/'):
