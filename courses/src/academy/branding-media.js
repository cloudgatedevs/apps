...
import { isCoursesFolder, loadMediaPages } from './media-model';

export async function saveBrandImage(blob, name, path = 'courses/branding') {
...
export async function listCoursesImages() {
  const files = await loadMediaPages(options => preview ? call('list', options, true, 'branding-media') : listImages(options));
  return files.filter(file => isCoursesFolder(file.path));
}

export function removeCoursesImage(id) {
