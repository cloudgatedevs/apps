...
import { listCoursesImages, removeCoursesImage } from './branding-media';
...
export default function MediaPanel({ data, go }) {
  ...
  const [uploading, setUploading] = useState(false), [uploadFolder, setUploadFolder] = useState('courses/covers'), [queue, setQueue] = useState([]), [uploaded, setUploaded] = useState(0);
  const generation = useRef(0), mutation = useRef(false);
  const refresh = useCallback(async () => {
    const request = ++generation.current;
    setLoading(true); setError('');
    try {
      const [next, current] = await Promise.all([listCoursesImages(), call('admin-data', {}, true)]);
  ...
  const remove = async () => {
    ...
    try {
      ...
      const [currentFiles, currentData] = await Promise.all([listCoursesImages(), call('admin-data', {}, true)]);
      const candidates = deletionCandidates(confirm.map(file => file.id), currentFiles, currentData);
      const failures = [];
      for (const file of candidates) {
        try { await removeCoursesImage(file.id); deleted++; }
  ...
  return <section className="media-page" aria-label="Courses media library">
    <div className="panel media-intro"><div><span className="eyebrow">YOUR IMAGE LIBRARY</span><h2>A home for your academy images.</h2><p>Upload images once, then choose them from Library when editing a course, instructor or your branding.</p><div className="media-counts"><span><ImageIcon size={15}/>{`${rows.length} image${rows.length === 1 ? '' : 's'}`}</span><span><Check size={15}/>{rows.length - unused.length} in use</span><span><FolderOpen size={15}/>{unused.length} unused</span></div></div><Button onClick={() => { setError(''); setUploading(true); }} disabled={loading || deleting}><Upload size={16}/> Upload images</Button></div>
    ...
    <p className="media-footnote">This page manages images in the Courses folders. Usage reflects saved courses, instructors and branding in the current academy environment; published curriculum images remain protected. Images bundled with the app are managed with its source files.</p>
