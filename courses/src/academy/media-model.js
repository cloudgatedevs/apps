...
export const MEDIA_FOLDERS = { 'courses/covers': 'Course covers', 'courses/instructors': 'Instructors', 'courses/branding': 'Branding', 'courses/library': 'General library' };
export const folderLabel = path => MEDIA_FOLDERS[path] || path.slice('courses/'.length);
export function isCoursesFolder(path) {
  return typeof path === 'string' && path.startsWith('courses/') && path.split('/').every(part => part && part !== '.' && part !== '..' && !part.includes('\\'));
}

function imageKeys(value) {
  if (!value) return [];
  try {
    const url = new URL(String(value).trim(), 'https://courses.invalid');
...
export function mediaRows(files, data) {
  if (!Array.isArray(data?.services) || !data?.settings) throw new Error('Reload academy data before managing media.');
  const references = (data.mediaReferences || []).map(r => ({keys:imageKeys(r.url),label:r.label,page:'courses'}));
  for (const person of data.staff || []) references.push({ keys: imageKeys(person.image_url), label: person.name + ' (team)', page: 'instructors' });
  for (const service of data.services) {
    // Retired services no longer display their photo. Hidden services still need theirs.
    if (Number(service.deleted)) continue;
    references.push({ keys: imageKeys(service.image_url), label: service.name + (Number(service.active) ? '' : ' (hidden)'), page: 'courses' });
  }
  for (const [key, label] of Object.entries(SETTINGS)) references.push({ keys: imageKeys(data.settings[key]), label, page: 'settings' });
  return files.filter(file => isCoursesFolder(file.path)).map(file => {
...
export function deletionCandidates(ids, files, data) {
  ...
  return [...new Set(ids.map(String))].map(id => {
    ...
    if (!row) throw new Error('An image is no longer in the Courses library. Refresh and select your images again.');
    if (row.inUse) throw new Error(`${row.name} is now in use. Remove it from its course, instructor or branding setting before deleting it.`);
    return row;
  });
}

// The host caps each page. Read every page before filtering by app folder so
// academy images remain visible even when other apps have a large library.
