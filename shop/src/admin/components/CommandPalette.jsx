...
export const CommandPalette = ({ open, onOpenChange }) => {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      const [orders, products, customers] = await Promise.all([
        adminApi.orders.list({ search: term, take: 5 }).catch(() => null),
        adminApi.products.list({ search: term, take: 5, status: 'all' }).catch(() => null),
  ...
  const items = useMemo(() => {
    ...
    const orders = results.orders.map((o) => ({ kind: 'Order', label: o.Reference, sub: `${[o.Name, o.Surname].filter(Boolean).join(' ') || o.Email} · ${fmtCents(o.TotalCents, o.Currency)} · ${o.Status}`, to: `/orders/${o.Id}`, key: `o-${o.Id}` }));
    const products = results.products.map((p) => ({ kind: 'Product', label: p.Name, sub: [p.Sku, p.Status].filter(Boolean).join(' · '), to: `/products/${p.Id}`, key: `p-${p.Id}` }));
    const customers = results.customers.map((c) => ({ kind: 'Customer', label: [c.Name, c.Surname].filter(Boolean).join(' ') || c.Email, sub: c.Email, to: `/customers/${c.Id}`, key: `c-${c.Id}` }));
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="ui-dialog card fixed left-1/2 top-[12vh] z-50 w-[min(40rem,calc(100vw-2rem))] -translate-x-1/2 overflow-hidden p-0 shadow-pop outline-none" onKeyDown={onKey}>
          ...
          <div className="flex items-center gap-2 border-b border-ink-700 px-4">
            ...
            <input ref={inputRef} value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search orders, products, customers…" className="h-12 grow bg-transparent text-sm text-mist outline-none placeholder:text-mist-dim" />
