...
import { listJobsImages, removeJobsImage } from './branding-media';
...
export default function MediaPanel({ data, go }) {
  ...
  const [uploading, setUploading] = useState(false), [uploadFolder, setUploadFolder] = useState('jobs/services'), [queue, setQueue] = useState([]), [uploaded, setUploaded] = useState(0);
  const generation = useRef(0), mutation = useRef(false);
  const refresh = useCallback(async () => {
    const request = ++generation.current;
    setLoading(true); setError('');
    try {
      const [next, current] = await Promise.all([listJobsImages(), call('admin-data', {}, true)]);
  ...
  const remove = async () => {
    ...
    try {
      ...
      const [currentFiles, currentData] = await Promise.all([listJobsImages(), call('admin-data', {}, true)]);
      const candidates = deletionCandidates(confirm.map(file => file.id), currentFiles, currentData);
      const failures = [];
      for (const file of candidates) {
        try { await removeJobsImage(file.id); deleted++; }
  ...
  return <section className="media-page" aria-label="Jobs media library">
    ...
    <p className="media-footnote">This page manages images in the Jobs folders. Usage reflects saved services, team members and branding in the current jobs environment; deleted services release their photos for cleanup. Images bundled with the app are managed with its source files.</p>
