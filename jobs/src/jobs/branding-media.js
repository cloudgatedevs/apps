...
import { isJobsFolder, loadMediaPages } from './media-model';

export async function saveBrandImage(blob, name, path = 'jobs/branding') {
...
export async function listJobsImages() {
  const files = await loadMediaPages(options => preview ? call('list', options, true, 'branding-media') : listImages(options));
  return files.filter(file => isJobsFolder(file.path));
}

export function removeJobsImage(id) {
