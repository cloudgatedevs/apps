...
export const MEDIA_FOLDERS = { 'jobs/services': 'Services', 'jobs/team': 'Team', 'jobs/branding': 'Branding', 'jobs/library': 'General library' };
export const folderLabel = path => MEDIA_FOLDERS[path] || path.slice('jobs/'.length);
export function isJobsFolder(path) {
  return typeof path === 'string' && path.startsWith('jobs/') && path.split('/').every(part => part && part !== '.' && part !== '..' && !part.includes('\\'));
}

function imageKeys(value) {
  if (!value) return [];
  try {
    const url = new URL(String(value).trim(), 'https://jobs.invalid');
...
export function mediaRows(files, data) {
  if (!Array.isArray(data?.services) || !data?.settings) throw new Error('Reload jobs data before managing media.');
  const references = [];
  ...
  return files.filter(file => isJobsFolder(file.path)).map(file => {
...
export function deletionCandidates(ids, files, data) {
  ...
  return [...new Set(ids.map(String))].map(id => {
    ...
    if (!row) throw new Error('An image is no longer in the Jobs library. Refresh and select your images again.');
...
// jobs photos remain visible even when other apps have a large library.
