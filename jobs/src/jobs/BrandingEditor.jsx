...
export default function BrandingEditor({ value, change }) {
  ...
  return <>
    <section className="panel settings-group"><h3>Branding</h3><p className="muted">Your identity across the jobs site, workspace and browser tab.</p><div className="form-grid"><Field label="App name" hint="Leave blank to use your business name."><input value={value.app_name} maxLength={120} placeholder={value.name || 'Jobs'} onChange={e => change('app_name', e.target.value)}/></Field><Field label="Short name" hint="Used when customers add the site to their home screen."><input value={value.app_short_name} maxLength={30} placeholder={brand.name} onChange={e => change('app_short_name', e.target.value)}/></Field></div>
    ...
    <section className="panel settings-group"><h3>Theme colours</h3><p className="muted">The default palette is deep green, fresh green and white. Choose your own colours below; button labels and body text adapt to keep them readable.</p><div className="brand-colours"><Colour label="Primary colour" value={value.theme_primary} change={v => change('theme_primary', v)}/><Colour label="Accent colour" value={value.theme_accent} change={v => change('theme_accent', v)}/><Colour label="Page background" value={value.theme_background} change={v => change('theme_background', v)}/></div><button type="button" className="text-link" onClick={() => ['theme_primary', 'theme_accent', 'theme_background'].forEach(key => change(key, BRAND_DEFAULTS[key]))}>Restore default colours</button>
    <div className="branding-preview" style={themeTokens(value)} aria-label="Branding preview"><div className="brand-browser-tab"><img src={brand.favicon} alt=""/><span>{brand.name}</span><small>Preview</small></div><div className="brand-preview-body"><Brand settings={value} small/><span className="eyebrow">GOOD WORK, MADE SIMPLE</span><h2>{value.tagline || 'Your next project, in good hands.'}</h2><p>{value.description || 'Reliable services for your home and business.'}</p><div className="actions"><span className="button">Request a quote <ArrowUpRight size={15}/></span><span className="brand-preview-tag"><Leaf size={15}/> From start to finish</span></div></div></div><p className="muted brand-preview-note">Preview only. Save business settings to apply your changes.</p></section>
  </>;
}
