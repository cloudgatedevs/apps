...
import { X, ArrowUpRight, LoaderCircle, Leaf, Check } from 'lucide-react';
...
export function Brand({ settings, small = false }) {
 ...
 return <a className={'brand ' + (small ? 'small' : '')} href="/" aria-label={brand.name + ' home'} onClick={settings ? e => e.preventDefault() : undefined}>{brand.logo && !failed ? <img className="brand-logo" src={brand.logo} alt={brand.showName ? '' : brand.name} onError={() => setFailed(true)}/> : <span className="brand-mark"><Leaf size={20}/></span>}{(brand.showName || failed) && <span className="brand-name">{brand.name}</span>}</a>;
}
...
export function Empty({ title, children }) { return <div className="empty"><Leaf size={32}/><h3>{title}</h3><p>{children}</p></div>; }
