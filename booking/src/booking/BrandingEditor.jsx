...
function CropImage({ file, close, select, square }) {
  ...
  const [aspect, setAspect] = useState(square ? 1 : 3), [busy, setBusy] = useState(false), [error, setError] = useState('');
  ...
  const upload = async original => {
    ...
    try {
      ...
      const result = await saveBrandImage(blob, file.name);
  ...
  return <Modal title="Prepare your image" wide close={() => { if (!busy) close(); }}>
    ...
    <div className="form-grid"><Field label="Zoom"><input type="range" min="1" max="3" step="0.05" value={zoom} onChange={e => setZoom(Number(e.target.value))}/></Field><Field label="Crop shape"><select value={aspect} onChange={e => setAspect(Number(e.target.value))}><option value="1">Square</option><option value="3">Wide logo (3:1)</option><option value="2">Landscape (2:1)</option></select></Field></div>
    <ErrorBox error={error}/><div className="actions"><Button type="button" busy={busy} disabled={!area} onClick={() => upload(false)}>Upload cropped image</Button><Button type="button" secondary disabled={busy} onClick={() => upload(true)}>Use entire image</Button></div>
  </Modal>;
}
...
function Asset({ label, hint, value, change, square }) {
  ...
  return <div className="brand-asset"><h4>{label}</h4><p className="muted">{hint}</p><ImagePreview url={imageUrl(value)} label={label}/>
    ...
    {file && <CropImage file={file} square={square} close={() => setFile(null)} select={change}/>}{library && <MediaLibrary close={() => setLibrary(false)} select={change}/>}
  </div>;
}
