...
export function brandIdentity(settings = {}) {
  ...
  return { name: s.app_name.trim() || s.name.trim() || 'Booking', shortName: s.app_short_name.trim() || s.app_name.trim() || s.name.trim() || 'Booking', logo: s.logo_url, icon: s.icon_url || s.favicon_url || s.logo_url || '/booking-icon.svg', favicon: s.favicon_url || s.icon_url || s.logo_url || '/booking-icon.svg', showName: s.logo_show_name !== '0' || !s.logo_url };
}
